...
export async function uploadToS3Direct({ kind, fileOrBlob, filename, contentType, courseId, sectionId }) {
  ...
  await s3.send(new PutObjectCommand({ Bucket: bucket, Key: key, Body: fileOrBlob, ContentType: contentType || 'application/octet-stream', ...(cacheControl ? { CacheControl: cacheControl } : {}) }))
