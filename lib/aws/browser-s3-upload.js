...
function buildKey({ kind, courseId, sectionId, identityId, filename }) {
  ...
  // Store all files directly in their respective folders (thumbnails/, videos/, files/)
...
export async function uploadToS3Direct({ kind, fileOrBlob, filename, contentType, courseId, sectionId }) {
  ...
  try {
    const invalidationResponse = await fetch('/api/cloudfront/invalidate', {
      method: 'POST',
    ...
    if (invalidationResponse.ok) {
      const result = await invalidationResponse.json().catch(() => ({}))
      if (result.skipped) {
        console.info('CloudFront invalidation skipped:', result.message || 'Credentials not configured')
      }
    } else {
      const errorData = await invalidationResponse.json().catch(() => ({}))
      console.warn('CloudFront invalidation failed:', {
    ...
    // Don't fail the upload if invalidation fails
  }
