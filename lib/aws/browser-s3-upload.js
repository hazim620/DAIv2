...
export async function uploadToS3Direct({ kind, fileOrBlob, filename, contentType, courseId, sectionId }) {
  ...
  if (!region) {
    const available = Object.keys(process.env).filter(k => k.startsWith('NEXT_PUBLIC_'))
    console.error('Missing NEXT_PUBLIC_APP_AWS_REGION. Available NEXT_PUBLIC_ vars:', available)
    throw new Error('Missing environment variable: NEXT_PUBLIC_APP_AWS_REGION. Make sure it is set in Amplify and the app has been rebuilt.')
  }
  if (!bucket) {
    throw new Error('Missing environment variable: NEXT_PUBLIC_APP_S3_BUCKET. Make sure it is set in Amplify and the app has been rebuilt.')
  }
  if (!cloudfrontDomain) {
    throw new Error('Missing environment variable: NEXT_PUBLIC_APP_CLOUDFRONT_DOMAIN. Make sure it is set in Amplify and the app has been rebuilt.')
  }
  if (!identityPoolId) {
    throw new Error('Missing environment variable: NEXT_PUBLIC_APP_COGNITO_IDENTITY_POOL_ID. Make sure it is set in Amplify and the app has been rebuilt.')
  }
